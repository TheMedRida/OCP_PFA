...
import com.Elito.OCP.kafka.server.PredictionService;
...
public class SensorDataConsumer {
    ...
    public void consume(String message) {
        try {
            ...
            // IGNORE THE EXISTING PREDICTION FIELDS ( to do later )
            // reading.setFailureProbability(data.path("Failure_Probability").asDouble());
            // reading.setRul(data.path("RUL").asDouble());
            // reading.setTtf(data.path("TTF").asDouble());

            // NEW: ADD ML PREDICTIONS
            Map<String, Object> sensorDataMap = convertJsonToMap(data);
            Map<String, Object> prediction = predictionService.predictFailure(sensorDataMap);

            // Set the new ML prediction columns
            reading.setMlFailureProbability((Double) prediction.get("mlFailureProbability"));
            reading.setMlPredictedFailure((Boolean) prediction.get("mlPredictedFailure"));
            reading.setMlModelVersion((String) prediction.get("mlModelVersion"));
            reading.setMlConfidenceLevel((String) prediction.get("mlConfidenceLevel"));
            reading.setMlPredictionLatencyMs((Integer) prediction.get("mlPredictionLatencyMs"));
            reading.setMlPredictionTimestamp(LocalDateTime.now().format(formatter));
